...
// Niches that use bold condensed typography
const CONDENSED_NICHES = ["barbershop", "barber", "salon", "hair"];

function isCondensedNiche(niche: string): boolean {
  const lower = niche.toLowerCase();
  return CONDENSED_NICHES.some((n) => lower.includes(n));
}
...
export default async function LeadDemoPage({
  ...
  // Fix 2: niche typography flag
  const useCondensed = isCondensedNiche(niche);

  return (
    <>
      {/*
       * Fix 1: Inject CSS custom properties on :root so every var(--color-primary)
       * and var(--color-accent) reference in this page resolves to Yoncé brand data.
       * Also override the hardcoded Tailwind yellow-400 classes used throughout.
       */}
      <style>{`
        :root {
        ...
        .hover\\:bg-yellow-300:hover { background-color: color-mix(in srgb, var(--color-primary) 88%, #fff) !important; }
        .border-yellow-400 { border-color: var(--color-primary) !important; }
        .border-yellow-400\\/30 { border-color: color-mix(in srgb, var(--color-primary) 30%, transparent) !important; }
        .bg-yellow-400\\/5 { background-color: color-mix(in srgb, var(--color-primary) 5%, transparent) !important; }
        .bg-yellow-400\\/20 { background-color: color-mix(in srgb, var(--color-primary) 20%, transparent) !important; }
        .bg-yellow-400\\/30 { background-color: color-mix(in srgb, var(--color-primary) 30%, transparent) !important; }
        .hover\\:border-yellow-400\\/50:hover { border-color: color-mix(in srgb, var(--color-primary) 50%, transparent) !important; }
        .shadow-\\[0_0_15px_rgba\\(250\\,204\\,21\\,0\\.3\\)\\] { box-shadow: 0 0 15px color-mix(in srgb, var(--color-primary) 30%, transparent) !important; }
        .shadow-\\[0_0_20px_rgba\\(250\\,204\\,21\\,0\\.2\\)\\] { box-shadow: 0 0 20px color-mix(in srgb, var(--color-primary) 20%, transparent) !important; }
        .hover\\:shadow-\\[0_0_30px_rgba\\(250\\,204\\,21\\,0\\.4\\)\\]:hover { box-shadow: 0 0 30px color-mix(in srgb, var(--color-primary) 40%, transparent) !important; }
        .shadow-\\[0_0_30px_rgba\\(250\\,204\\,21\\,0\\.3\\)\\] { box-shadow: 0 0 30px color-mix(in srgb, var(--color-primary) 30%, transparent) !important; }
        .decoration-yellow-400\\/30 { text-decoration-color: color-mix(in srgb, var(--color-primary) 30%, transparent) !important; }
      `}</style>
      ...
      <div className="min-h-screen bg-[#0a0a0a] text-white selection:bg-yellow-400 selection:text-black font-sans">
        ...
        <nav className="fixed top-0 w-full z-50 border-b border-white/10 bg-black/50 backdrop-blur-xl px-12 py-6 flex justify-between items-center">
          ...
          {/* Fix 4: encoded subject + businessName interpolated correctly */}
          <Link
            href={`mailto:hello@flynerd.tech?subject=${encodeURIComponent(`Launch ${businessName} Website`)}`}
        ...
        <section className="pt-48 pb-24 px-12 max-w-7xl mx-auto">
          <div className="flex flex-col lg:flex-row gap-16 items-center">
            <div className="flex-1 space-y-8">
              ...
              {/*
               * Fix 2: Apply Bebas Neue for barbershop/salon niches.
               * The class is always imported at module level; className is
               * conditionally applied so non-barber niches stay unaffected.
               */}
              <h1
                className={`text-6xl lg:text-8xl font-black tracking-tight leading-[1.1] ${
                  useCondensed ? bebasNeue.className : ""
                }`}
              >
                A New Era for <br />
              ...
              <p className="text-xl text-neutral-400 max-w-2xl leading-relaxed">
                We took your business details and built a conversion-focused web portal
                designed specifically for {niche} professionals. No more generic templates —
                this is built to turn your local visibility into booked clients.
              </p>
        ...
        {/* Fix 5: Pain Points — rendered dynamically from intelData.painPoints */}
          ...
          <div className="max-w-7xl mx-auto space-y-12 text-center">
            <div className="space-y-4">
              ...
              <p className="text-neutral-400">
                We identified these critical areas where the new demo outperforms your
                current presence:
              </p>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-4">
              {painPoints.map((point: string, i: number) => (
                <div
                  key={i}
                  className="flex items-center gap-3 px-8 py-4 rounded-full bg-neutral-900 border border-white/5 hover:border-yellow-400/50 transition-colors shadow-lg shadow-black/20"
                >
        ...
        <section className="py-32 px-12 text-center max-w-4xl mx-auto space-y-12">
          ...
          <p className="text-xl text-neutral-400 leading-relaxed max-w-2xl mx-auto">
            This demo is live and ready to be customized with your actual brand colors,
            testimonials, and booking links. We can have the full version launched in as
            little as 48 hours.
          </p>
          <div className="flex flex-col sm:flex-row justify-center gap-6">
            {/* Fix 4: businessName correctly interpolated in mailto subject */}
            <Link
              href={`mailto:hello@flynerd.tech?subject=${encodeURIComponent(`Launch ${businessName} Website`)}`}
            ...
            {/* Fix 3: Book a Strategy Call wired to GOOGLE_MEET_BOOKING_LINK */}
            <Link
              href={BOOKING_LINK}
              target="_blank"
      ...
      {/* Fix 6: Floating Claude-powered chat widget — one exchange then locked CTA */}
      <DemoChatWidget
