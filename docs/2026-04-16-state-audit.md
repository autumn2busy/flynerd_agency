...
| 2.1 | Scrub `flynerdtech` legacy. Correct global support email. | **Not done.** 4 files still have `flynerdtech.com`: `app/terms/page.tsx:112`, `app/privacy/page.tsx:75,91`, `app/contact/page.tsx:215-216`, `app/api/contact/route.ts:178` (send-from). Meanwhile newer files use `flynerd.tech` inconsistently. | NOT STARTED | Grep `flynerdtech\.com` in `app/**/*.tsx,ts`. |
...
| 3 | Legal string for Footers / TOS (`flynerd-agency` vs LLC name)? | 🔴 **UNANSWERED.** Terms and Privacy still say `flynerdtech.com` (`app/terms/page.tsx:112`, `app/privacy/page.tsx:23,75,91`). | Grep `flynerdtech\.com`. |
...
| 9 | Correct global support email? | 🔴 **UNANSWERED AND CONTRADICTORY.** `hello@flynerdtech.com`, `info@flynerdtech.com`, and `hello@flynerd.tech` all appear in production files. `app/api/contact/route.ts:178-179` has BOTH `flynerdtech.com` (send-from) and `flynerd.tech` (send-to) in the same function. | Grep above. |
...
- 🔴 **Domain rename (`flynerdtech.com` → `flynerd-agency.com` or wherever).** Decision #3 is open. Inconsistent email is annoying but does not block a paid lead.
