"use client";

import { Star } from "lucide-react";
import type { NicheBullet } from "./nicheConfig";
import { getNicheColors } from "./nicheConfig";

interface Props {
  businessName: string;
  niche: string;
  nicheBullets: NicheBullet[];
  painPoints: string[];
  rating: number;
  reviewCount: number;
  rawServices?: string;
  bookCtaHref: string;
}

export default function SitePreview({
  businessName,
  niche,
  nicheBullets,
  painPoints,
  rating,
  reviewCount,
  rawServices,
  bookCtaHref,
}: Props) {
  const colors = getNicheColors(niche);
  const services = nicheBullets.slice(0, 3);

  const heroTagline = rawServices
    ? `${rawServices.split(/[,\n]/)[0].trim()} and more — delivered right.`
    : painPoints[0]
    ? `No more ${painPoints[0].toLowerCase()}. Just results.`
    : `Built for ${niche} businesses that are serious about growth.`;

  const bookLabel =
    niche.toLowerCase().includes("cleaning") ||
    niche.toLowerCase().includes("hvac") ||
    niche.toLowerCase().includes("plumb") ||
    niche.toLowerCase().includes("roofing") ||
    niche.toLowerCase().includes("landscap")
      ? "Get a Free Estimate"
      : "Book a Consultation";

  const fakeHost = businessName.toLowerCase().replace(/[^a-z0-9]/g, "") + ".com";

  return (
    <section className="py-24 px-6 md:px-12 border-t border-white/5">
      <div className="max-w-6xl mx-auto">
        <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-center mb-3">
          A preview of your new site
        </h2>
        <p className="text-center text-neutral-400 mb-10">
          This is what we'd build for you — tuned to your niche and customers.
        </p>

        {/* Browser chrome wrapper */}
        <div className="rounded-2xl overflow-hidden border border-white/10 shadow-2xl">
          {/* Chrome bar */}
          <div className="bg-[#1c1c1e] border-b border-white/10 px-4 py-3 flex items-center gap-3">
            <div className="flex gap-1.5 shrink-0">
              <span className="w-3 h-3 rounded-full bg-[#FF5F57]" />
              <span className="w-3 h-3 rounded-full bg-[#FEBC2E]" />
              <span className="w-3 h-3 rounded-full bg-[#28C840]" />
            </div>
            <div className="flex-1 bg-[#2c2c2e] rounded-md px-3 py-1.5 text-xs text-neutral-500 font-mono text-center truncate">
              {fakeHost}
            </div>
          </div>

          {/* Simulated website — light theme */}
          <div className="bg-white text-neutral-900 font-sans">
            {/* Site header */}
            <header
              className="px-6 py-4 flex items-center justify-between border-b"
              style={{ borderColor: `${colors.accent}44` }}
            >
              <span
                className="font-black text-sm tracking-tight"
                style={{ color: colors.primary }}
              >
                {businessName.toUpperCase()}
              </span>
              <nav className="hidden md:flex gap-6 text-xs text-neutral-500 font-medium">
                <span>Services</span>
                <span>About</span>
                <span>Reviews</span>
                <span>Contact</span>
              </nav>
              <button
                className="text-xs font-bold px-4 py-2 rounded-full text-white"
                style={{ background: colors.primary }}
              >
                Book Now
              </button>
            </header>

            {/* Hero */}
            <div
              className="px-6 py-10 md:py-14 text-center"
              style={{ background: colors.bg }}
            >
              <h1
                className="text-2xl md:text-4xl font-black tracking-tight mb-3 leading-tight"
                style={{ color: colors.text }}
              >
                {businessName}
              </h1>
              <p className="text-sm md:text-base text-neutral-500 mb-6 max-w-lg mx-auto">
                {heroTagline}
              </p>
              {rating > 0 && (
                <div className="flex items-center justify-center gap-1.5 mb-6">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      size={13}
                      fill={i < Math.round(rating) ? colors.primary : "transparent"}
                      stroke={colors.primary}
                    />
                  ))}
                  <span className="text-xs text-neutral-400 ml-1">
                    {rating.toFixed(1)}{reviewCount > 0 ? ` · ${reviewCount} reviews` : ""}
                  </span>
                </div>
              )}
              <button
                className="text-sm font-bold px-6 py-3 rounded-full text-white shadow-md"
                style={{ background: colors.primary }}
              >
                {bookLabel}
              </button>
            </div>

            {/* Services grid */}
            <div
              className="px-6 py-10 border-t"
              style={{ borderColor: `${colors.accent}33` }}
            >
              <h2
                className="text-sm font-bold text-center mb-6 uppercase tracking-widest"
                style={{ color: colors.primary }}
              >
                What We Do
              </h2>
              <div className="grid md:grid-cols-3 gap-4">
                {services.map((bullet, i) => (
                  <div
                    key={i}
                    className="p-4 rounded-xl border text-left"
                    style={{ borderColor: `${colors.primary}22`, background: colors.bg }}
                  >
                    <p
                      className="text-xs font-bold mb-1.5"
                      style={{ color: colors.primary }}
                    >
                      {bullet.title}
                    </p>
                    <p className="text-xs text-neutral-500 leading-relaxed">
                      {bullet.desc}
                    </p>
                  </div>
                ))}
              </div>
            </div>

            {/* Testimonial strip */}
            <div
              className="px-6 py-8 text-center border-t"
              style={{
                background: `${colors.primary}0d`,
                borderColor: `${colors.accent}33`,
              }}
            >
              <p className="text-xs italic text-neutral-500 max-w-md mx-auto mb-2">
                {`"Best experience I've had. Booked same day and they showed up on time. Highly recommend ${businessName}."`}
              </p>
              <p className="text-xs font-bold" style={{ color: colors.primary }}>
                — Verified Customer
              </p>
            </div>

            {/* Sticky CTA bar */}
            <div
              className="px-6 py-5 flex items-center justify-between border-t bg-white"
              style={{ borderColor: `${colors.accent}33` }}
            >
              <div>
                <p className="text-xs font-black text-neutral-800">Ready to go live?</p>
                <p className="text-xs text-neutral-400">Full site delivered in 7 days.</p>
              </div>
              <a
                href={bookCtaHref}
                target="_blank"
                rel="noopener noreferrer"
                className="text-xs font-bold px-5 py-2.5 rounded-full text-white"
                style={{ background: colors.primary }}
              >
                Book Your Build
              </a>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
